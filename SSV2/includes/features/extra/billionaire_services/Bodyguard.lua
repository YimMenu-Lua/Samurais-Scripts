...
---@return handle?
function Bodyguard.spawn(modelHash, name, spawnPos, weapon, godmode, noRagdoll, behavior)
...
---@param behavior? integer
---@return Bodyguard?
function Bodyguard.new(ref, modelHash, name, spawnPos, weapon, godmode, noRagdoll, behavior)
	local handle = Bodyguard.spawn(modelHash, name, spawnPos, weapon, godmode, noRagdoll, behavior)
...
function Bodyguard:IsInPrivateHeli()
	...
	local heli  = self.m_owner_ref.ActiveServices.heli
		...
		if (playerElevation >= 5 or playerElevation < 0) then
			if (not PV:IsLandVehicle() and not PV:IsPedInVehicle(self.m_handle)) then
				...
				if (self:IsBodyguard()) then
					BSV2:DismissBodyguard(self)
				else
					BSV2:DismissEscortGroup(self.escortGroup.name)
				end
...
function Bodyguard:GrabNearestVehicle()
	...
	local function IsSuitableVehicle(veh)
		...
		return cls
			...
			and not cls:IsHeli()
			and not cls:IsPlane()
			and not BSV2:GetEscortVehicleByHandle(veh)
