...
function PrivateJet.spawn(ref, model, airportData)
	...
	local jet = setmetatable(
		{
			...
			pilot         = pilot,
			copilot       = copilot,
			pilotName     = ref:GetRandomPedName(Enums.ePedGender.MALE),
			copilotName   = ref:GetRandomPedName(Enums.ePedGender.MALE),
			blip          = {
				handle = blip,
				alpha = 255
			},
