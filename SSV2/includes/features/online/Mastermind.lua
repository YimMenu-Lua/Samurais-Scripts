...
local secondary_targets = { "CASH", "WEED", "COKE", "GOLD", "PAINT" }
...
---@field m_tab Tab
local Mastermind   = { m_raw_data = require("includes.data.yrv3_data"), __label = "Mastermind" }
...
function Mastermind:SetCayoSecTargets(type, index)
	...
	if (index) then targets[index] = -1 end

	for i = 1, 5 do
		local stat_name = _F("MPX_H4LOOT_%s_%s", secondary_targets[i], type)
		stats.set_int(stat_name, targets[i])
		stats.set_int(_F("%s_SCOPED", stat_name), targets[i])
	end
end
...
function Mastermind:GetCayoSecTargets()
	...
	for i = 1, 5 do
		local stat_name = _F("MPX_H4LOOT_%s", secondary_targets[i])
...
function Mastermind:ResetCayoPerico()
	...
	for _, str in ipairs({ "I", "C" }) do
		self:SetCayoSecTargets(str)
	end
