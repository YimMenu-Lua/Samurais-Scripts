...
local IManagedValueController <const> = require("includes.services.IManagedValueController")
local RawData <const>                 = require("includes.data.yrv3_data")
local MpProperties <const>            = require("includes.data.mp_properties")
...
---@enum eBusinessMgrState
Enums.eBusinessMgrState = {
	IDLE      = 0x0,
	OFFLINE   = 0x1,
	WAITING   = 0x2,
	LOADING   = 0x3,
...
---@field private m_bhub_thread_id number
...
---@field private m_businesses MpBusinesses
...
function BusinessMgr:init()
	...
	self.m_state                     = Enums.eBusinessMgrState.IDLE
...
---@param reason? string
function BusinessMgr:Reset(disable, reason)
	self.m_total_sum                 = 0
	self.m_total_sum_fmt             = "$0"
	self.m_last_autosell_check_time  = 0
	self.m_last_income_check_time    = 0
	...
	self.m_data_initialized          = false
	self.m_autosell_state            = Enums.eAutoSellState.NONE
	self.m_state                     = disable and Enums.eBusinessMgrState.ERROR or Enums.eBusinessMgrState.IDLE
...
function BusinessMgr:Reload()
	if (self.m_state == Enums.eBusinessMgrState.RELOADING) then
		return
	end

	self.m_state = Enums.eBusinessMgrState.RELOADING
...
---@return boolean
function BusinessMgr:CanAccess()
	return Backend:IsUpToDate()
		and Game.IsOnline()
		and not Backend:IsMockEnv()
		and not NETWORK.NETWORK_IS_ACTIVITY_SESSION()
		and self.m_state ~= Enums.eBusinessMgrState.ERROR
end

---@return boolean
function BusinessMgr:IsDataInitialized()
	return self.m_data_initialized
end

---@return eBusinessMgrState
function BusinessMgr:GetState()
	return self.m_state
end

---@return string
function BusinessMgr:GetLastError()
...
---@param msg string
function BusinessMgr:SetLastError(msg)
...
function BusinessMgr:BusinessIter()
	return pairs(self.m_businesses)
end

function BusinessMgr:GetAvailableBossTypes()
	return self.m_boss_types_avail
end

---@return Office?
function BusinessMgr:GetOffice()
	return self.m_businesses.office
end

---@return Warehouse?
function BusinessMgr:GetHangar()
	return self.m_businesses.hangar
end

---@return Clubhouse?
function BusinessMgr:GetClubhouse()
	return self.m_businesses.clubhouse
end

---@return Nightclub?
function BusinessMgr:GetNightclub()
	return self.m_businesses.nightclub
end

---@return array<CashSafe>
function BusinessMgr:GetBusinessSafes()
	return self.m_businesses.safes
end

---@return CarWash?
function BusinessMgr:GetCarWash()
	return self.m_businesses.car_wash
end

---@return Factory?
function BusinessMgr:GetBunker()
	return self.m_businesses.bunker
end

---@return Factory?
function BusinessMgr:GetAcidLab()
	return self.m_businesses.acid_lab
end

---@return SalvageYard
function BusinessMgr:GetSalvageYard()
	return self.m_businesses.salvage_yard
end

---@return dict<{ type: "float"|"bool", tuneables: array<string> }>
function BusinessMgr:GetSaleMissionTunables()
	return RawData.SellMissionTunables
end

---@return integer
function BusinessMgr:GetEstimatedIncome()
	return self.m_total_sum
end

---@return string
function BusinessMgr:GetEstimatedIncomeFmt()
	return self.m_total_sum_fmt
end

---@return boolean
function BusinessMgr:IsAnySaleInProgress()
	return self.m_sell_script_running
end

---@return eAutoSellState
function BusinessMgr:GetAutoSellState()
...
---@param slot integer
---@return boolean
function BusinessMgr:DoesPlayerOwnPropertySlot(stat, slot)
	return stats.get_int(_F("%s%d", stat, slot)) ~= 0
end

---@return boolean
function BusinessMgr:DoesPlayerOwnAnyWarehouse()
	for i = 0, 4 do
...
---@return boolean
function BusinessMgr:DoesPlayerOwnAnyBikerBusiness()
	for i = 0, 4 do
...
---@param idx integer
---@return boolean
function BusinessMgr:IsPropertyIndexValid(idx)
...
function BusinessMgr:PopulateOffice()
	...
	local ref = MpProperties.Offices[idx]
...
function BusinessMgr:PopulateClubhouse()
	...
	local club_ref = MpProperties.Clubhouses[idx]
...
function BusinessMgr:PopulateNightclub()
	...
	ThreadManager:Run(function()
		...
		local ref      = MpProperties.Nightclubs[nc_index]
...
function BusinessMgr:PopulateCarWash()
...
function BusinessMgr:PopulateCashSafes()
	...
	for i, data in ipairs(RawData.CashSafes.regular) do
		...
		local entry                = MpProperties[data.property_key][property_index]
...
function BusinessMgr:PopulateSalvageYard()
	local property_index = stats.get_int("MPX_SALVAGE_YARD_OWNED")
	if (not self:IsPropertyIndexValid(property_index)) then
		return
	end

	local ref       = MpProperties.SalvageYards[property_index]
...
function BusinessMgr:InitializeData()
	if (self.m_data_initialized
			or not Game.IsOnline()
			or self.m_state == Enums.eBusinessMgrState.LOADING
			or self.m_state == Enums.eBusinessMgrState.RELOADING
		) then
		return
	end

	self.m_state      = Enums.eBusinessMgrState.LOADING
	self.m_last_error = "GENERIC_WAIT_LABEL"
	self:PreInit()
	self:PopulateNightclub()
end

function BusinessMgr:CommandFinishSale()
...
---@param index number
function BusinessMgr:CommandWarehouseAutoFill(index)
	if (not self:CanAccess()) then
		return
	end
...
function BusinessMgr:CommandHangarAutoFill()
	if (not self:CanAccess()) then
		return
	end
...
---@return Factory?
function BusinessMgr:GetFactoryByIndex(index)
...
---@param index integer -- `1 .. 7`
function BusinessMgr:CommandFactoryRestock(index)
...
function BusinessMgr:CommandToggleProduction(index, isNightclubHub)
...
---@return array<CashSafe>
function BusinessMgr:GetAllSafes()
	local outArray = {}
...
function BusinessMgr:FillAllSafes()
	if (not GVars.features.unsafe_feats_enabled) then
		Notifier:ShowError("Business Manager", _T("YRV3_UNSAFE_FEAT_BYPASS_ERR"))
		return
	end
...
---@return string
function BusinessMgr:GetRunningSellScriptDisplayName()
...
function BusinessMgr:FinishSale(doCleanup)
...
function BusinessMgr:FinishCEOCargoSourceMission()
...
---@param name string
function BusinessMgr:ProcessCooldown(name)
	self.m_cooldown_controller:SetDirty(name, true)
end

function BusinessMgr:ProcessAllCooldowns()
	self.m_cooldown_controller:SetNextCallForced(true)
end

function BusinessMgr:CooldownHandler()
...
function BusinessMgr:FindRunningSaleScript()
...
function BusinessMgr:SetupAutosell()
	...
	if (self.m_sell_script_running and self.m_bhub_thread_id ~= 0) then -- was triggered from the mct
...
function BusinessMgr:AutoSellHandler()
	self:SetupAutosell()
...
function BusinessMgr:MCT()
	...
	ThreadManager:Run(function(s)
		...
		self.m_bhub_thread_id = SYSTEM.START_NEW_SCRIPT("appArcadeBusinessHub", Enums.eScriptStackSize.DEFAULT)
		...
		while (script.is_active("appArcadeBusinessHub")) do
			...
			if (self:IsAnySaleInProgress()) then
				break
			end
...
function BusinessMgr:UpdateBusinesses()
	if (not self:CanAccess()) then
		return
	end
...
function BusinessMgr:CalculateEstimatedIncome()
...
function BusinessMgr:OnTick()
	yield()

	if (not Backend:IsUpToDate()) then
		self.m_state = Enums.eBusinessMgrState.ERROR
	...
	if (not self.m_data_initialized and not self.m_initial_data_done and self.m_state ~= Enums.eBusinessMgrState.LOADING) then
		self:InitializeData()
	end
	...
	self.m_state = Enums.eBusinessMgrState.RUNNING
	self:SetLastError("")
