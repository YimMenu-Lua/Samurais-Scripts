...
function BusinessHub:SetProductCount(count)
	self.m_tech_idx_g:At(8):At(1, self.m_id):WriteInt(count)
...
function BusinessHub:TriggerProduction(count)
	...
	if (techIdx == -1 or current == max) then
		return
	end
...
function BusinessHub:HasFullProduction()
	return self:GetProductCount() >= self.m_max_units -- greater or equal because you never know when things could break
end

---@return boolean
function BusinessHub:CanTriggerProduction()
	return self:GetTimeLeftBeforeProd() > 1000 and not self:HasFullProduction()
end
