...
local drawTxnPopup            = require("includes.frontend.business_mgr.helpers.withdraw_deposit_popup")
...
local function drawPortfolio()
	...
	drawKeyValue(_T("YRV3_INCOME_APPROX_ALL"), BusinessMgr:GetEstimatedIncomeFmt(), colMoneyGreen)
	ImGui.SetWindowFontScale(1.0)
	ImGui.PopStyleColor()
	ImGui.Separator()
	ImGui.Spacing()
...
return function()
	...
	if (LocalPlayer:IsBoss()) then
		...
		local property = isMC and BusinessMgr:GetClubhouse() or BusinessMgr:GetOffice()
		bossLabel      = _T(fmt, property and property:GetCustomName() or "GENERIC_UNKNOWN")
	else
		bossLabel = _T(LocalPlayer:IsAssociate() and "YRV3_DASHBOARD_BOSS_ASSOCIATE" or "YRV3_DASHBOARD_BOSS_RETIRED")
	end
