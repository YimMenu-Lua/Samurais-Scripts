...
local BikerBusiness   = setmetatable({}, BusinessBase)
BikerBusiness.__index = BikerBusiness

---@param opts BikerOpts
---@return BikerBusiness
function BikerBusiness.new(opts)
...
---@return integer
function BikerBusiness:GetProductValue()
	return math.floor(self:GetProductCount() * self.m_vpu)
end

function BikerBusiness:ReStock()
...
---@return milliseconds
function BikerBusiness:GetTimeLeftBeforeProd()
...
function BikerBusiness:TriggerProduction()
	local g_obj      = SGSL:Get(SGSL.data.biker_prod_time_global)
	local pid_size   = g_obj:GetOffset(1)
	local offset_2   = g_obj:GetOffset(2)
	local offset_3   = g_obj:GetOffset(3)
...
---@return boolean
function BikerBusiness:IsFull()
	return self:GetProductCount() == self.m_max_units
end

---@return boolean
function BikerBusiness:HasEquipmentUpgrade()
	return self.m_equipment_upgrade
end

---@return boolean
function BikerBusiness:HasStaffUpgrade()
	return self.m_staff_upgrade
end

---@return boolean
function BikerBusiness:CanTriggerProduction()
	return self:GetTimeLeftBeforeProd() > 1000
end

function BikerBusiness:LoopProduction()
