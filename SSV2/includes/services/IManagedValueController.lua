...
function IManagedValueController:Append(name, data_t)
	...
	local ok, res = pcall(IManagedValueEntry.new, name, data_t)
	if (not ok) then
		log.fwarning("[IManagedValueController]: Failed to register managed valie '%s': %s", name, res)
		return false
	end
...
function IManagedValueController:Clear()
	self:Reset()
	self.m_items = {}
end

function IManagedValueController:OnCall()
	if (self.m_running or not self.m_initialized) then
		return
	end

	self:ForEach(function(entry)
		entry:OnCall(self.m_force_next_call)
	end)
