...
---@class CasinoPacino
---@field cooldown_update_str string
...
function CasinoPacino:GiveWheelPrize(prizeID)
    if (not script.is_active("casino_lucky_wheel")) then
        Toast:ShowError("CasinoPacino", _T("CP_MUST_BE_AT_WHEEL"))
        return
    end
...
function CasinoPacino:ForcePokerCards()
    ...
    if (players_current_table ~= -1) then -- If the player is sitting at a poker table
        local player_0_card_1 = locals.get_int("three_card_poker", (three_card_poker_cards)
            + (three_card_poker_current_deck)
            + (1 + (players_current_table * three_card_poker_deck_size))
            + (2) + (1) + (0 * 3)
        )

        local player_0_card_2 = locals.get_int("three_card_poker", (three_card_poker_cards)
            + (three_card_poker_current_deck)
            + (1 + (players_current_table * three_card_poker_deck_size))
            + (2) + (2) + (0 * 3)
        )

        local player_0_card_3 = locals.get_int("three_card_poker", (three_card_poker_cards)
            + (three_card_poker_current_deck)
            + (1 + (players_current_table * three_card_poker_deck_size))
            + (2) + (3) + (0 * 3)
        )
...
function CasinoPacino:ForceRouletteWheel()
    ...
    if (not script.is_active("casinoroulette")) then
        return
    end
...
function CasinoPacino:SetCooldownString()
    ...
    self.cooldown_update_str        = chipswon_gd >= max_chip_wins and _F(_T("CP_COOLDOWN_BYPASS_STATUS_FORMAT"), minutes_left) or _T("CP_COOLDOWN_BYPASS_STATUS_OFF")
end

---@return string
function CasinoPacino:GetCooldownString()
    return self.cooldown_update_str
end
...
function CasinoPacino:Start()
    ...
    if GVars.features.dunk.bypass_casino_bans then
        self:BypassCooldown()
    end

    if GVars.features.dunk.force_poker_cards then
        self:ForcePokerCards()
    end

    if GVars.features.dunk.force_roulette_wheel then
