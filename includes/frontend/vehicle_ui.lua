...
local function QuickConfigWindow(label, callback, onClose)
    ...
    ImGui.SetWindowPos(center.x, center.y, ImGuiCond.Once | ImGuiWindowFlags.NoSavedSettings | ImGuiWindowFlags.NoCollapse)
...
local function speedoOptions()
    ...
    ImGui.Text("Colors")
    ImGui.Separator()
    GVars.features.speedometer.colors.circle,      _ = ImGui.ColorEditU32("Circle Color", GVars.features.speedometer.colors.circle)
    GVars.features.speedometer.colors.circle_bg,   _ = ImGui.ColorEditU32("Circle Background Color", GVars.features.speedometer.colors.circle_bg)
    GVars.features.speedometer.colors.text,        _ = ImGui.ColorEditU32("Text Color", GVars.features.speedometer.colors.text)
    GVars.features.speedometer.colors.markings,    _ = ImGui.ColorEditU32("Markings Color", GVars.features.speedometer.colors.markings)
    GVars.features.speedometer.colors.needle,      _ = ImGui.ColorEditU32("Needle Color", GVars.features.speedometer.colors.needle)
    GVars.features.speedometer.colors.needle_base, _ = ImGui.ColorEditU32("Needle Base Color", GVars.features.speedometer.colors.needle_base)
...
local function driftOptions()
    ...
    GVars.features.vehicle.drift.power, _ = ImGui.SliderInt("Torque Increase",  GVars.features.vehicle.drift.power, 10, 100)

    ImGui.BeginDisabled(GVars.features.vehicle.drift.mode ~= 1)
    GVars.features.vehicle.drift.intensity, _ = ImGui.SliderInt("Strong Mode Intensity", GVars.features.vehicle.drift.intensity, 1, 3)
    ImGui.EndDisabled()
end
...
vehicleTab:AddBoolCommand("NOS", "features.vehicle.nos.enabled", nil, nil, nil, true) -- description is used for both the UI tooltip and the command description in the CommandExecutor window
vehicleTab:AddBoolCommand("NOS Purge", "features.vehicle.nos.purge", nil, nil, { description = "placeholder" }, true)
vehicleTab:AddBoolCommand("Pops & Bangs", "features.vehicle.burble_tune", nil, RestoreExhaustPops, { description = "placeholder" }, true)
vehicleTab:AddBoolCommand("Drift Mode", "features.vehicle.drift.enabled", nil, nil, { description = "placeholder" }, true)
vehicleTab:AddBoolCommand("Big Subwoofer", "features.vehicle.subwoofer", nil, ToggleSubwoofer(false), { description = "placeholder" }, true)
vehicleTab:AddBoolCommand("High Beams on Horn", "features.vehicle.horn_beams", nil, nil, { description = "placeholder" }, true)
vehicleTab:AddBoolCommand("Auto Brake Lights", "features.vehicle.auto_brake_lights", nil, nil, { description = "placeholder" }, true)
...
vehicleTab:AddBoolCommand("Flappy Doors", "features.vehicle.flappy_doors", nil, CloseDoors, { description = "placeholder" }, true)
vehicleTab:AddBoolCommand("Auto Lock Doors", "features.vehicle.auto_lock_doors", nil, nil, { description = "placeholder" }, true)
vehicleTab:AddBoolCommand("Launch Control", "features.vehicle.launch_control", nil, nil, { description = "Simulates launch control. Only available for performance cars." }, true)
...
end, { description = "Imitates GTA IV's vehicle exit style: Hold [F] for one second to turn off the engine or normal press to leave it running." }, true)
