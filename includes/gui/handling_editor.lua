...
function handingEditorUI()
    ...
    if self.get_veh() ~= 0 then
        rocketBoost, rbUsed = ImGui.Checkbox("Rocket Boost", Memory.GetVehicleModelFlag(Self.Vehicle.Current, VMF._HAS_ROCKET_BOOST))
        ...
        ImGui.SameLine(); ImGui.Dummy(25, 1); ImGui.SameLine();
        vehJump, vjUsed = ImGui.Checkbox("Vehicle Jump", Memory.GetVehicleModelFlag(Self.Vehicle.Current, VMF._JUMPING_CAR))
        ...
        if vehJump then
            ImGui.SameLine(); ImGui.Dummy(28, 1); ImGui.SameLine(); ImGui.BeginDisabled(not Self.Vehicle.IsCar)
            vehChute, vehChuteUsed = ImGui.Checkbox("Parachute", Memory.GetVehicleModelFlag(Self.Vehicle.Current, VMF._HAS_PARACHUTE))
            ImGui.EndDisabled()
        ...
        rwSteering, rwstrUsed = ImGui.Checkbox("Rear Wheel Steering", Memory.GetVehicleHandlingFlag(Self.Vehicle.Current, HF._STEER_REARWHEELS))
