...
function vehicleUI()
    ...
    ImGui.SameLine()
    ImGui.BeginDisabled(Self.IsDriving() or not Self.IsOutside() or Self.Vehicle.Current == 0)
    if ImGui.Button("TP Into Last Vehicle") then
        UI.widgetSound("Select")
        script.run_in_fiber(function()
            if ENTITY.DOES_ENTITY_EXIST(Self.Vehicle.Current) and ENTITY.IS_ENTITY_A_VEHICLE(Self.Vehicle.Current) and
                VEHICLE.IS_VEHICLE_DRIVEABLE(Self.Vehicle.Current, true) then
                TASK.CLEAR_PED_TASKS_IMMEDIATELY(Self.GetPedID())
                PED.SET_PED_INTO_VEHICLE(Self.GetPedID(), Self.Vehicle.Current, -1)
            else
    ...
    if ImGui.Button("Bring Last Vehicle") then
        ...
        script.run_in_fiber(function(blv)
            if ENTITY.DOES_ENTITY_EXIST(Self.Vehicle.Current) and ENTITY.IS_ENTITY_A_VEHICLE(Self.Vehicle.Current) and
                VEHICLE.IS_VEHICLE_DRIVEABLE(Self.Vehicle.Current, true) then
                local veh_coords  = ENTITY.GET_ENTITY_COORDS(Self.Vehicle.Current, true)
                ...
                if distance <= 15 then
                    ...
                    local self_fwd   = ENTITY.GET_ENTITY_FORWARD_VECTOR(Self.GetPedID())
                    local veh_hash   = ENTITY.GET_ENTITY_MODEL(Self.Vehicle.Current)
                    ...
                    ENTITY.SET_ENTITY_COORDS(
                        Self.Vehicle.Current, tp_offset.x, tp_offset.y,
    ...
    if t_SpeedometerWindow.should_draw then
        ...
        speedometercircle_color = {Col(speedometer.circle_color):ToFloat()}
        ...
        speedometertext_color = {Col(speedometer.text_color):ToFloat()}
        ...
        speedometermarkings_color = {Col(speedometer.markings_color):ToFloat()}
        ...
        speedometerneedle_color = {Col(speedometer.needle_color):ToFloat()}
        ...
        speedometerneedle_base_color = {Col(speedometer.needle_base_color):ToFloat()}
...
function DrawSpeedometer(
    ...
    if should_draw_speedometer then
        ...
        if ImGui.Begin(
            ...
            -- fractions
            for i = 0, max_speed, 1 do
                local angle = start_angle + (end_angle - start_angle) * (i / max_speed)
            ...
            for i = 0, max_speed, 20 do
                ...
                -- analog needle
                local speed_ratio = math.min(current_speed / max_speed, 1)
                ...
                local num_active_segments = math.floor(30 * (Self.Vehicle.RPM))
